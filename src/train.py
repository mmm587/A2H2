...
#     model = XLNetForSequenceClassification.from_pretrained(hyp_params.model, num_labels=1, ignore_mismatched_sizes=True)
...
#     test_accuracies = []
#     best_valid = 1e8
#     for epoch in range(1, hyp_params.n_epochs + 1):  # 每一轮 都如下进行训练
#         # 训练 验证 与 测试
...
def train(hyp_params, train_loader, valid_loader, test_loader, num_train_optimization_steps):
    settings = initiate(hyp_params, num_train_optimization_steps)
    metrics_tracker = MetricsTracker(hyp_params.sequence_model)
    ...
    for epoch in range(1, hyp_params.n_epochs + 1):
        ...
        train_loss = train_model(settings, hyp_params, train_loader)
        epoch_time = metrics_tracker.stop_timer(mode='train')

        valid_loss = evaluate_model(settings, hyp_params, valid_loader)

        print("\n" + "-" * 50)
        print(f'Epoch {epoch:2d}| Train Loss {train_loss:5.4f} | Valid Loss {valid_loss:5.4f}')
        print(f'Training time: {epoch_time:.2f}s')
        print("-" * 50)

        results, truths = test_model(settings['model'], hyp_params, test_loader)
        acc, mae, corr, f_score = test_score_model(results, truths, metrics_tracker=metrics_tracker)
    ...
    print('The test results:')
    print('| MAE {:5.4f} | Corr {:5.4f} | F1 {:5.4f} | ACC {:5.4f}'.format(
        min(test_mae), max(test_corr), max(test_f_score), max(test_accuracies)
    ))
    print(f'Total training time: {metrics_tracker.metrics_history["train_time"]:.2f}s')
    print(f'Average inference time: {metrics_tracker.metrics_history["inference_time"] / hyp_params.n_epochs:.2f}s')
